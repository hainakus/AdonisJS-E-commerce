    ...
    if (isAdmin.id == request.currentUser.id) {
        const id = request.param('id');
          const user = yield User
            ...
            .with('Items.products')
            .where('users.id', '=', id).fetch()
        const sql = yield Database.raw("SELECT p.product_id, s.title, s.description, s.price, sum(p.quantity) as SumProductQuantity FROM products S JOIN items p on S.id = p.product_id WHERE p.quantity > 1 GROUP BY s.title, s.description, s.price,  p.product_id")
